...
def encryptString(msg_string: str, publicKey: int, n: int):
    return [encryptInt(token, publicKey, n) for token in longMsgIterator(msg_string)]

def decryptString(msg_list: list, privateKey: int, n: int):
    token_combiner = tokensCombiner()
    for token in msg_list: token_combiner.addToken(token,privateKey,n)
    return convertIntegerToString(token_combiner.getNum())


############################# YOU SHOULD NOT READ & MODIFY FUNCTIONS BELOW ################################

def convertStringToInteger(obj: str):
    # FIXME: It is known that this function's output is not deterministic. (same input of 'testMsg' will give out
    #  different answer) IMPORTANT - This defact will lead to failure of project directly.
    return int.from_bytes(str.encode('ascii'), byteorder="little")

def convertIntegerToString(obj: int):
    max_len, curr_len = 1024 ** 3, 64
    while curr_len < max_len:
        try:
            resObject = obj.to_bytes(curr_len, byteorder='little').decode('ascii')
            break
...
class longMsgIterator:
    def __init__(self, msg: object):
        self.message = msg
        if type(msg) == str: self.msgInt = convertStringToInteger(msg)
        else: self.msgInt = convertObjectToInteger(msg)
...
if __name__ == "__main__":
    for i in range(50):
        intRes = convertStringToInteger("test1")
        print(intRes)
        tokens = [token for token in longMsgIterator("test1")]
        print(tokens)
